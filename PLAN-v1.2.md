...
- [ ] `alembic revision --autogenerate` run after every model change
- [ ] `pytest -v` passes before every commit
...
- [ ] `Device` — hostname, ip_address, device_type enum, vendor, model, location, status enum, last_seen
- [ ] `Interface` — FK→Device CASCADE, name, ip_address, mac_address, speed_mbps, status enum
- [ ] `MetricSample` — FK→Device CASCADE, FK→Interface SET NULL (optional), metric_type enum, value, sampled_at
- [ ] `Alert` — FK→Device CASCADE, severity enum, title, description, status enum, acknowledged_at, resolved_at
- [ ] `NetworkConfig` — FK→Device CASCADE, config_text, config_hash, captured_at, notes, compliance_notes

### 0.2 Alembic Initial Migration
**File:** `backend/alembic/versions/<hash>_initial_schema.py`
- [ ] `alembic revision --autogenerate -m "initial_schema"` generates all 5 tables
- [ ] Migration verified (`alembic upgrade head` runs clean)
...
- [ ] `InterfaceCreate`, `InterfaceUpdate`, `InterfaceRead`
- [ ] `MetricSampleCreate`, `MetricSampleRead`, `MetricQuery` (device_id, metric_type, since, until)
- [ ] `AlertCreate`, `AlertUpdate`, `AlertRead`
- [ ] `NetworkConfigCreate`, `NetworkConfigRead`
- [ ] `DashboardStats` (total_devices, online_count, offline_count, degraded_count, open_alerts, critical_alerts, avg_latency_ms)
...
- [ ] `DeviceRepository` — `filter_by_status()`, `filter_by_type()`, `search(q)` (hostname/IP ILIKE)
- [ ] `InterfaceRepository` — `list_by_device(device_id)`
- [ ] `MetricRepository` — `query_range(device_id, type, since, until)`, `latest_per_device(device_ids, type)`, `avg_latency_last_hour()`
- [ ] `AlertRepository` — `list_open()`, `list_by_device(device_id)`, `count_by_status()`
- [ ] `ConfigRepository` — `list_by_device(device_id)` ordered by captured_at desc
...
- [ ] `GET/POST /api/v1/devices/{device_id}/interfaces`, `GET/PUT/DELETE /api/v1/devices/{device_id}/interfaces/{id}`
- [ ] `POST /api/v1/metrics` (ingest), `GET /api/v1/metrics` (query by device/type/range)
- [ ] `GET/POST /api/v1/alerts`, `GET/PUT/DELETE /api/v1/alerts/{id}` (PUT handles acknowledge/resolve)
- [ ] `GET/POST /api/v1/configs`, `GET /api/v1/devices/{device_id}/configs`
- [ ] `GET /api/v1/dashboard` → `DashboardStats`

### 0.6 Wire Routers in main.py
**File:** `backend/app/api/main.py`
- [ ] All v1 routers included with correct prefixes
...
- [ ] Device CRUD lifecycle + search/filter tests
- [ ] Interface CRUD under device
- [ ] Metric ingest + range query
- [ ] Alert CRUD + acknowledge/resolve transitions
- [ ] Config capture + list by device
- [ ] Dashboard stats correctness

### 0.8 DPanel Manifest
**File:** `frontend/public/dclaw-manifest.json`
- [ ] `{ "app_id": "network", "name": "DClaw Network", "backend_port": 8044, "frontend_port": 3044, "db": "dclaw_network", "version": "0.1.0" }`

### 0.9 Frontend API Client
**File:** `frontend/src/lib/api.ts`
- [ ] TypeScript interfaces for all entities
- [ ] Typed fetch functions for every endpoint

### 0.10 Dashboard Page
**File:** `frontend/src/app/page.tsx`
- [ ] Real stats from `GET /api/v1/dashboard` (device counts, open alerts, avg latency)
- [ ] Recent critical alerts feed
- [ ] Uses pre-built Card, Badge, Table components

### 0.11 Devices List Page
**File:** `frontend/src/app/devices/page.tsx`
- [ ] Table with search (hostname/IP) and filter chips (status, type)
- [ ] "Add Device" Dialog form
- [ ] Delete with confirmation

### 0.12 Alerts Page
**File:** `frontend/src/app/alerts/page.tsx`
- [ ] Table sorted by severity + created_at
...
- [ ] Background task polling MetricSamples every 60s
- [ ] Auto-creates Alerts: latency_ms>200→warning, >500→critical; packet_loss>2%→warning, >10%→critical; cpu_pct>80%→warning, >95%→critical
- [ ] Deduplicates: skips if open alert for same device+metric already exists

### 1.2 Bulk Metric Ingest
**File:** `backend/app/api/v1/metrics.py`
- [ ] `POST /api/v1/metrics/bulk` — list of MetricSampleCreate, inserted via `session.add_all()`
...
- [ ] Device info card (edit/delete)
- [ ] Interfaces table
- [ ] Last-1h metric sparklines (latency, packet loss, CPU)
- [ ] Active alerts list
- [ ] Config history timeline

### 1.5 Performance Page
**File:** `frontend/src/app/performance/page.tsx`
- [ ] Multi-device metric comparison
- [ ] Time-range selector: 1h / 6h / 24h / 7d
- [ ] Top-N worst performers table

### 1.6 Configuration Page
**File:** `frontend/src/app/configs/page.tsx`
- [ ] Config history per device
- [ ] Text diff viewer between two snapshots

### 1.7 Navigation Layout
**File:** `frontend/src/components/AppLayout.tsx`
- [ ] Sidebar: Dashboard, Devices, Alerts, Performance, Configurations, Topology
- [ ] Active route highlighting
- [ ] Wraps all pages via root `layout.tsx`

### 1.8 Slack / Webhook Integration
**File:** `backend/app/services/webhook_service.py`
- [ ] POST to `SLACK_WEBHOOK_URL` env var on critical alert creation
- [ ] Add `SLACK_WEBHOOK_URL` to `.env.example` and `config.py`
...
- [ ] `POST /api/v1/copilot/chat` — context-aware LLM chat (Ollama primary, OpenRouter fallback)
- [ ] Builds context from last 1h metrics + active alerts for referenced device
- [ ] Streaming response (text/event-stream)
- [ ] Floating chat widget on every page (bottom-right, Dialog)
- [ ] **Demo query**: "Which devices had packet loss > 5% last hour?"
...
- [ ] On alert creation: send last 30min metrics + alert history to LLM
- [ ] "Explain root cause in 2-3 sentences. Suggest one remediation step."
- [ ] Stores result in `Alert.description`
- [ ] **YC "wow" moment**: alert description auto-populated in plain English
...
4. 1.7 → 1.4 → 1.5 → 1.6 → 1.1 → 1.2 → 1.3 → 1.8 (layout first, then rich pages, then services)
5. 2.3 → 2.1 → 2.2 → 2.4 → 2.5 → 2.6 (RCA first since it enhances Complexity 1 alerts)
